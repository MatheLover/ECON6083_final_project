...
IV Regression
```{r}
iv <- ivreg(lneduyear ~ lnjinshipop+lnnightlight+lncoastdist+tri+suitability+lnpopqing+urbanrates+as.factor(provid)+rvrdist|rvrdist+bprvdist+lnnightlight+lncoastdist+tri+suitability+lnpopqing+urbanrates+as.factor(provid),data=census)
iv_ROBUST <- coeftest(iv,vcov=vcovHC(iv, type = "HC0"))
stargazer(iv_ROBUST, type="text")

# First stage
first_stage <- lm(lnjinshipop ~ rvrdist+bprvdist+lnnightlight+lncoastdist+tri+suitability+lnpopqing+urbanrates+as.factor(provid),data=census )
first_stage <- coeftest(first_stage,vcov=vcovHC(iv, type = "HC0"))
